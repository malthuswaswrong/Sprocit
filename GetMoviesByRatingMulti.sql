...
/****** Object: SqlProcedure [dbo].[GetMoviesByRatingMulti] Script Date: 10/5/2024 12:59:47 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE GetMoviesByRatingMulti
    @MinRating FLOAT
AS
BEGIN
    ...
    -- First result set: movies meeting the rating threshold
    SELECT Movie_ID, Title, Release_Year, Genre, Director, Duration, Rating
    FROM #Movies
    WHERE Rating >= @MinRating;

    -- Second result set: movies below the rating threshold
    SELECT Movie_ID, Title, Release_Year, Genre, Director, Duration, Rating
    FROM #Movies
    WHERE Rating < @MinRating;
